...
# Uncraft recipe
execute as @a at @s as @e[name=ccl_gct,type=armor_stand,tag=old,distance=..20] at @s if block ~ ~1 ~ dropper{Items:[{Slot: 4b, id: "minecraft:armor_stand", Count: 1b, tag:{display:{Name:"{\"text\":\"Golden Crafting Table\",\"color\":\"gold\"}",Lore:['{"text":"A Custom Crafting Table for your own new and customizable crafting recipes.","color":"aqua"}']},EntityTag:{CustomNameVisible:0b,NoGravity:1b,Invulnerable:1b,Invisible:1b,NoBasePlate:1b,DisabledSlots:4144959,Tags:["new","vm_entity","vm_block_machine","vm_block_machine+1"],ArmorItems:[{},{},{},{id:"minecraft:light_weighted_pressure_plate",Count:1b}],CustomName:"{\"text\":\"ccl_gct\",\"color\":\"gold\"}"}}}]} run tag @s add ccl_uncraft_r
